...
from app.schemas.ledger import LedgerResponse, LedgerEntry, LedgerSummary
...
# ============================================================================
# LEDGER / ACCOUNT STATEMENT ENDPOINTS
# ============================================================================
...
async def get_supplier_ledger(
    ...
    """Get monthly ledger for a specific supplier (purchases + payments)"""
    ...
    # Get all purchases for this supplier in the specified month
    purchases = db.query(Purchase).filter(
    ...
    # Calculate opening balance (total owed from previous months)
    previous_purchases = db.query(Purchase).filter(
        Purchase.supplier_id == supplier_id,
        Purchase.date < datetime(year, month, 1)
    ).all()
    ...
    for purchase in purchases:
        amount_owed = float(purchase.total_amount)
        amount_paid = float(purchase.paid_amount or 0)
        ...
        # Create entry for purchase
        transactions.append(LedgerEntry(
            date=purchase_date,
            reference_number=purchase.bill_number,
            description=f"Purchase - {purchase.notes or 'Supplies'}",
            debit=amount_owed,
            credit=0.0,
            balance=running_balance + amount_owed,
            payment_status=purchase.payment_status,
            transaction_type="purchase"
        ))
        
        total_debit += amount_owed
        running_balance += amount_owed
        
        # If payment was made, add payment entry
        if amount_paid > 0:
            transactions.append(LedgerEntry(
                date=purchase_date,
                reference_number=f"{purchase.bill_number}-PAY",
                description=f"Payment - {purchase.bill_number}",
                debit=0.0,
                credit=amount_paid,
                balance=running_balance - amount_paid,
                payment_status="paid",
...
async def get_customer_ledger(
    ...
    """Get monthly ledger for a specific customer (sales + payments received)"""
    ...
    # Get all sales for this customer in the specified month
    sales = db.query(Sale).filter(
    ...
    # Calculate opening balance (total owed by customer from previous months)
    previous_sales = db.query(Sale).filter(
        Sale.customer_id == customer_id,
        Sale.date < datetime(year, month, 1)
    ).all()
    
    opening_balance = sum([
        float(s.total_price) - float(s.paid_amount or 0)
    ...
    for sale in sales:
        amount_owed = float(sale.total_price)
        amount_paid = float(sale.paid_amount or 0)
        ...
        # Create entry for sale (customer owes us)
        transactions.append(LedgerEntry(
            date=sale_date,
            reference_number=sale.bill_number,
            description=f"Sale - {sale.notes or 'Bottles/Supplies'}",
            debit=amount_owed,
            credit=0.0,
            balance=running_balance + amount_owed,
        ...
        # If payment was received, add payment entry
        if amount_paid > 0:
            transactions.append(LedgerEntry(
                date=sale_date,
                reference_number=f"{sale.bill_number}-PAY",
                description=f"Payment Received - {sale.bill_number}",
                debit=0.0,
                credit=amount_paid,
                balance=running_balance - amount_paid,
                payment_status="paid",
...
async def get_supplier_ledger_pdf(
    ...
    """Generate professional PDF ledger for supplier using Platypus"""
    from reportlab.lib.pagesizes import A4
    from reportlab.lib import colors
    ...
    # Get purchases and calculate data
    purchases = db.query(Purchase).filter(
        Purchase.supplier_id == supplier_id,
        extract('year', Purchase.date) == year,
        extract('month', Purchase.date) == month
    ...
    for purchase in purchases:
        ...
        # Purchase entry
        date_str = purchase.date.strftime("%d-%m-%Y") if hasattr(purchase.date, 'strftime') else str(purchase.date)
        transactions.append({
            'date': date_str,
        ...
        if amount_paid > 0:
            transactions.append({
                'date': date_str,
                'reference': f"{purchase.bill_number}-PAY",
                'description': 'Payment',
                'debit': 0,
                'credit': amount_paid,
                'balance': running_balance - amount_paid
    ...
    # Top blue line
    blue_line = Table([['']],  colWidths=[7*inch], rowHeights=[0.05*inch])
    ...
    # Header
    header_style = ParagraphStyle('HeaderStyle', parent=styles['Normal'], fontSize=11, textColor=colors.black)
    story.append(Paragraph("<b>Waze Enterprises - Water Bottle Division</b>", header_style))
    story.append(Paragraph("Street S-6, Rawat Industrial Area, Rawalpindi", header_style))
    story.append(Paragraph("Phone: 0343-9998954", header_style))
    story.append(Spacer(1, 10))
    
    # Title
    title_style = ParagraphStyle('TitleStyle', parent=styles['Heading1'], fontSize=16, textColor=colors.HexColor('#0066CC'), spaceAfter=10)
    story.append(Paragraph(f"SUPPLIER LEDGER - {calendar.month_name[month]} {year}", title_style))
    ...
    story.append(Paragraph(f"<b>Supplier:</b> {supplier.name}", info_style))
    story.append(Paragraph(f"<b>ID:</b> {supplier_id}", info_style))
    if supplier.contact:
        story.append(Paragraph(f"<b>Contact:</b> {supplier.contact}", info_style))
    story.append(Spacer(1, 12))
    
    # Summary cards
    summary_data = [
        ['Opening Balance', f"Rs {opening_balance:,.2f}"],
        ['Total Debit', f"Rs {total_debit:,.2f}"],
        ['Total Credit', f"Rs {total_credit:,.2f}"],
        ['Closing Balance', f"Rs {closing_balance:,.2f}"],
    ]
    summary_table = Table(summary_data, colWidths=[3*inch, 2.5*inch])
    summary_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (0, -1), colors.HexColor('#e9f3ff')),
    ...
    for txn in transactions:
        table_data.append([
            ...
            txn['reference'],
            txn['description'],
            f"Rs {txn['debit']:,.2f}" if txn['debit'] > 0 else '-',
            f"Rs {txn['credit']:,.2f}" if txn['credit'] > 0 else '-',
            f"Rs {txn['balance']:,.2f}",
        ])
    
    txn_table = Table(table_data, colWidths=[1*inch, 1.2*inch, 1.3*inch, 1*inch, 1*inch, 1.2*inch])
    ...
    filename = f"Ledger_{supplier.name}_{calendar.month_name[month]}_{year}.pdf"
    return StreamingResponse(
        buffer,
...
async def get_customer_ledger_pdf(
    ...
    """Generate professional PDF ledger for customer using Platypus"""
    from reportlab.lib.pagesizes import A4
    from reportlab.lib import colors
    from reportlab.lib.styles import ParagraphStyle, getSampleStyleSheet
    from reportlab.lib.units import mm, inch
    ...
    for sale in sales:
        ...
        # Sale entry
        date_str = sale.date.strftime("%d-%m-%Y") if hasattr(sale.date, 'strftime') else str(sale.date)
        transactions.append({
            'date': date_str,
        ...
        if amount_paid > 0:
            transactions.append({
                'date': date_str,
                'reference': f"{sale.bill_number}-PAY",
                'description': 'Payment',
                'debit': 0,
                'credit': amount_paid,
                'balance': running_balance - amount_paid
    ...
    # Title
    title_style = ParagraphStyle('TitleStyle', parent=styles['Heading1'], fontSize=16, textColor=colors.HexColor('#0066CC'), spaceAfter=10)
    story.append(Paragraph(f"CUSTOMER LEDGER - {calendar.month_name[month]} {year}", title_style))
    ...
    # Customer info
    info_style = ParagraphStyle('InfoStyle', parent=styles['Normal'], fontSize=10)
    story.append(Paragraph(f"<b>Customer:</b> {customer.name}", info_style))
    story.append(Paragraph(f"<b>ID:</b> {customer_id}", info_style))
    if customer.contact:
        story.append(Paragraph(f"<b>Contact:</b> {customer.contact}", info_style))
    story.append(Spacer(1, 12))
    ...
    filename = f"Ledger_{customer.name}_{calendar.month_name[month]}_{year}.pdf"
    return StreamingResponse(
        buffer,
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=\"{filename}\""}
