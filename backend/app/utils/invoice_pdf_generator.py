...
def generate_sales_invoice_pdf(sale_bill, customer, line_items, items_db):
    ...
    # Format date safely - use due_date if date is None
    invoice_date = sale_bill.date if sale_bill.date else sale_bill.due_date
...
def generate_purchase_invoice_pdf(purchase_bill, supplier, line_items, items_db):
    ...
    # Format date safely - use due_date if date is None
    invoice_date = purchase_bill.date if purchase_bill.date else purchase_bill.due_date
    if invoice_date and hasattr(invoice_date, 'strftime'):
        date_str = invoice_date.strftime('%d-%m-%Y')
    else:
        date_str = str(invoice_date) if invoice_date else "N/A"
